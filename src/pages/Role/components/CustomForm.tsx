...
import { GetRoleById, SaveRole, UpdateRole } from "../services/Services";
...
const ComponentsForm = ({ idEdit, onClearForm }: Props) => {
  ...
  useEffect(() => {
    const dataEditForm = async (token: string, idEdit: number) => {
      const dataRowEdit = await GetRoleById(token, idEdit);
      if (dataRowEdit.code === 200) {
        const data = dataRowEdit.data.roles[0];
        setStateBtn(false);
  ...
  const onSubmit = async (values: IForms) => {
    try {
      ...
      if (idEdit) {
        await UpdateRole(token, Number(idEdit), data);
      } else {
        await SaveRole(token, data);
      }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-3 gap-8 mt-4">
      <div className="col-span-1">
        <label className="block mb-2 text-sm font-medium text-gray-700">Nombre del rol</label>
        <input
          type="text"
